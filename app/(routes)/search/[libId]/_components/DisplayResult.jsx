...
// ─── Per-chat tab state ───────────────────────────────────────────────────────
// Each chat message tracks its own active tab to avoid shared state bug
// (switching tabs on one message was switching them all).
...
function ChatResult({ chat, index, streamingState, onFollowUp }) {
  ...
  // Memoize the extraction so it doesn't re-run on every parent render
...
function DisplayResult({ searchInputRecord }) {
  ...
  const addRateLimitToast = useCallback(() => {
    setToasts((prev) => [
      ...prev,
      { id: Date.now(), resetTime: getResetTime() },
    ]);
  }, []);
  ...
  useEffect(() => {
    ...
    if (shouldFetch) {
      ...
      runSearch(searchInputRecord.searchInput, selectedModelId);
    }
  ...
  // ── Core search ────────────────────────────────────────────────────────────
  // modelId is passed explicitly so the value is captured at call-time,
  // not read from a stale closure.
  const runSearch = useCallback(
    async (query, modelId = selectedModelId) => {
      ...
      try {
        const res = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            searchInput: query,
            searchResult: formattedSearchResp,
            modelId, // ← captured at call-time, not from a closure
          }),
        });
    ...
    [addRateLimitToast, libId, searchInputRecord?.type, selectedModelId],
  );
        ...
        <div className="pointer-events-auto w-full mx-4 sm:mx-8 md:mx-16 lg:mx-28 xl:mx-48 bg-white border border-gray-300 rounded-3xl shadow-sm px-4 py-3 flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              {/* ← Shared ModelPicker — same store, zero prop-drilling */}
              <ModelPicker />
              <Button
                variant="ghost"
                size="icon"
