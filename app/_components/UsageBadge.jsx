...
function StatusDot({ pct }) {
  const color = pct >= 90 ? "#ef4444" : pct >= 65 ? "#f59e0b" : "#22c55e"
  return <span className="inline-block w-1.5 h-1.5 rounded-full" style={{ background: color }} aria-hidden />
}
...
export function UsageBadge({ usage, mobileCompact = false }) {
  ...
  const dotColor    = searchPct >= 90 ? "#ef4444" : searchPct >= 65 ? "#f59e0b" : "#0d9488"
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {/* Hide "left" label on mobile compact to save space */}
      ...
      {/* ── Expanded panel — always anchors right on mobile ── */}
      {open && (
        <div
          role="dialog"
          aria-label="Usage details"
          className="absolute right-0 bottom-[calc(100%+8px)] z-50 w-56 rounded-xl border border-gray-100 bg-white p-3 shadow-xl shadow-black/[0.06]"
        >
          ...
          <div className="flex flex-col gap-1.5 mb-2.5">
            ...
            <Bar pct={searchPct} color="#0d9488" />
          </div>
          ...
          <div className="flex flex-col gap-1.5 mb-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-semibold text-gray-700">
                {research.used} / {research.limit}
              </span>
            </div>
            <Bar pct={researchPct} color="#7c3aed" />
          </div>
