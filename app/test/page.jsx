...
const PROVIDER_META = {
  ...
  gemini: {
    ...
    color: "#1d9bf0",
    bg: "#eff6ff",
    limits: "Flash-Lite: 15 RPM · 1K RPD · Flash: 10 RPM · 250 RPD",
  ...
  mistral: {
    ...
    color: "#ff7000",
    bg: "#fff7ed",
    limits: "~1B tok/month · rate-limited free tier",
    note: "EU-hosted · GDPR-compliant · includes Codestral",
  },
};
...
export default function TestPage() {
  ...
  async function loadModels() {
    ...
    try {
      ...
      if (data.errors && Object.keys(data.errors).length > 0) {
        setFetchErr(
          `Partial failure: ${Object.entries(data.errors)
            .map(([p, e]) => `${p}: ${e}`)
            .join(", ")}`,
        );
      }
  ...
  async function runTests() {
    ...
    setResults(
      Object.fromEntries(
        allModels.map((m) => [modelKey(m), { ...m, status: "loading" }]),
      ),
    );
  ...
  const errCount = allResults.filter(
    (r) => r.status === "error" || r.status === "empty",
  ).length;
  const okTimes = allResults
    .filter((r) => r.status === "ok" && r.ms)
    .map((r) => r.ms);
    ...
    return (
      <div style={styles.splash}>
        ...
        <div style={{ fontSize: 12, color: "#9ca3af" }}>
          Querying Groq · Cerebras · Gemini · NVIDIA · OpenRouter
        </div>
      </div>
    );
  ...
  return (
    <div style={styles.page}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {running && (
            <span style={{ fontSize: 13, color: "#6b7280" }}>
              {done}/{allModels.length}
            </span>
          )}
          <button
            onClick={runTests}
            disabled={!canRun}
            style={canRun ? styles.btnPrimary : styles.btnDisabled}
          >
      ...
      {hasResults && (
        ...
        >
          {[
            ...
            {
              val: avgMs ? `${(avgMs / 1000).toFixed(1)}s` : "—",
              label: "Avg response",
            },
          ].map(({ val, label }) => (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: "1.25rem" }}>
          {FILTERS.map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              style={filter === f ? styles.pillActive : styles.pill}
            >
              {f}
            </button>
          ))}
      ...
      {!hasResults && (
        ...
        >
          Hit "Run all models" to test {allModels.length} free-tier models
          across {groups.length} providers
        </div>
      )}
      ...
      {hasResults && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {groups.map(({ provider, meta, models: gModels }) => {
            ...
            const okN = gModels.filter(
              (m) => results[modelKey(m)]?.status === "ok",
            ).length;
            ...
            return (
              <div key={provider} style={styles.providerCard}>
                ...
                <div
                  ...
                  style={{ ...styles.providerHeader, background: meta.bg }}
                >
                  <div style={{ flex: 1 }}>
                    <div
                      style={{
